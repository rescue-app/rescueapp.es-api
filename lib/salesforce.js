...
class Salesforce {
    ...
    async insertAskFromForm(form, reference, test) {
        ...
        let offers = objects.filter(sobject => sobject[SalesforceFields.FIELD_TYPE] === 'Oferta');
        let needs = objects.filter(sobject => sobject[SalesforceFields.FIELD_TYPE] === 'Necesidad');
        ...
        askForm.stocks = [
            {
                ...
                details: test? `(From request ${askForm.reference}) - Test Data` : `(From request ${askForm.reference}) - ${originalObject[SalesforceFields.FIELD_DETAILS]}`,
    ...
    _getSalesforceSObjects(form) {        
        return form.stocks.map(stock => {
            return {
                [SalesforceFields.FIELD_ACCOUNT_NAME]: form.account,
                ...
                [SalesforceFields.FIELD_TYPE]: (stock.type === 'offer')? 'Oferta' : 'Necesidad',
            }
        });
    }
